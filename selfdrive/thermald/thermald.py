...
boot_file = '/data/openpilot/customboot'
...
def thermald_thread(end_event, hw_queue):
  ...
  while not end_event.is_set():
    ...
    custom_boot = params.get_bool("CustomBootScreen")
    if not os.path.isfile(boot_file) and custom_boot:
      os.system("cd /data/openpilot; touch customboot")
    elif os.path.isfile(boot_file) and not custom_boot:
      os.system("cd /data/openpilot; rm -f customboot")
